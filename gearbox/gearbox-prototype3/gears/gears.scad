 ...
 * шестерня 2: 15 зубов, шаг 2мм, угол зацепления 20
 * отверстие: круглый вал 2мм
 *
 * @param print_error погрешность при печати 3д-принтером (мм), стоит 
 * учитывать для элементов (отверстий или штырей), стыкующихся с 
...
module gear_transmission1(print_error=0) {
    ...
    union() {
      ...
      // шестеренка 2
      translate([0, 0, 2]) 
        linear_extrude(height=6) import(file="gear-pa20-cp2-t15.dxf");
 ...
 * 
 * шестерня 1: 38 зубов, шаг 2мм, угол зацепления 20 
 * шестерня 2: 15 зубов, шаг 2мм, угол зацепления 20
 * отверстие: круглый вал 2мм
 *
 * @param print_error погрешность при печати 3д-принтером (мм), стоит 
...
module gear_transmission2(print_error=0) {
    ...
    union() {
      // шестеренка 1
      linear_extrude(height=3) import(file="gear-pa20-cp2-t38.dxf");
      // шестеренка 2
      translate([0, 0, 2]) 
        linear_extrude(height=6) import(file="gear-pa20-cp2-t15.dxf");
      ...
      // подсадка, чтобы не расплывались зубья
      translate([0, 0, -0.3]) cylinder(h=1, r=11.1, $fn=100);
    }
 ...
 * 
 * 38 зубов, шаг 2мм, угол зацепления 20
