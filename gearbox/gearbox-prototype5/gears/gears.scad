...
// на Picaso 3D Designer с максимальным качеством 100% заполнения
...
/** 
 * Передача 1: с вала на передачу 2 (две шестерни).
 * 
 * шестерня 1: 47 зубов, шаг 1.5мм, угол зацепления 20 
 * шестерня 2: 12 зубов, шаг 1.5мм, угол зацепления 20
 * отверстие: круглый вал 2мм
 *
 * @param print_error погрешность при печати 3д-принтером (мм), стоит 
 * учитывать для элементов (отверстий или штырей), стыкующихся с 
...
module gear_transmission1(print_error=0) {
    ...
    union() {
      // шестеренка 1
      linear_extrude(height=3) import(file="gear-pa20-cp1.5-t47.dxf");
      // шестеренка 2
      translate([0, 0, 2]) 
        linear_extrude(height=6) import(file="gear-pa20-cp1.5-t12.dxf");

      // кругляш на макушке вокруг оси, чтобы упираться в стенку 0.5
      translate([0, 0, 8-0.1]) cylinder(h=0.5+0.1, r=2.3, $fn=100);

      // подсадка, чтобы не расплывались зубья
      translate([0, 0, -0.3]) cylinder(h=1, r=10.6, $fn=100);
    }
...
/** 
 * Передача 2: с передачи 1 на колесо (две шестерни).
 * 
 * шестерня 1: 47 зубов, шаг 1.5мм, угол зацепления 20 
 * шестерня 2: 12 зубов, шаг 1.5мм, угол зацепления 20
 * отверстие: круглый вал 2мм
 ...
 * и в него без учета погрешности может быть будет сложно втиснуть ось)
 */
module gear_transmission2(print_error=0) {
    difference() {
    union() {
      // шестеренка 1
      linear_extrude(height=3) import(file="gear-pa20-cp1.5-t47.dxf");
 ...
 * 
 * 47 зубов, шаг 1.5мм, угол зацепления 20
