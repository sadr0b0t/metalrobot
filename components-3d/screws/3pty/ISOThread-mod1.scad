...
module thread_out_centre(dia,hi,cq=$fn)
{
	p = get_coarse_pitch(dia);
	thread_out_centre_pitch(dia,hi,p,cq);
}

module thread_out_centre_pitch(dia,hi,p,cq=$fn)
{
    // cq=$fn - quality for centre cylinder ($fn by defaul)
    
	h = (cos(30)*p)/8;
    // with (5*h) thread spiral would not touch centre
    // and hang in the air
	//Rmin = (dia/2) - (5*h);	// as wiki Dmin
    Rmin = (dia/2) - (3*h);	// as wiki Dmin
	cylinder(r = Rmin, h = hi, $fn=cq);
}
